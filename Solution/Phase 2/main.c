...
void displayMainMenu(int [], double [], int *);     // displays the main menu
...
void printBooks(int [], double [], int);            // prints (displays) the book list (bins + prices) on the screen
...
int main(int argc, char **argv)
{
    ...
    displayMainMenu(bins, prices, &size);
...
void displayMainMenu(int bins[], double prices[], int *size)
{
    system("COLOR F9");
...
void uploadDataFile(int bins[], double prices[], int *size)
{
    ...
    while(!feof(dataFile))
    {
        fscanf(dataFile, "%d\t%lf\n", &bins[*size], &prices[*size]);
        (*size)++;
    }
    ...
    if(size > 0)
    {
        for(x=0, z=0; x<size; x++)
        {
            if(tempBin == bins[x])
            {
                ...
                gotoxy(10,8);
                printf("Book bin# is: %d , and its price is: %0.2f", bins[x], prices[x]);
...
void printBooks(int bins[], double prices[], int size)
{
    ...
    if(size > 0)
    {
        ...
        {
            Sleep(300);
            printf("\t\tBook bin# is: %d , and its price is: %0.2f\n", bins[x], prices[x]);
        }
    }
